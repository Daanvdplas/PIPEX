/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_split_pipex.c                                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dvan-der <dvan-der@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/14 13:55:25 by dvan-der          #+#    #+#             */
/*   Updated: 2021/12/14 13:55:27 by dvan-der         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

static void	free_splitted_arr(char **split_arr)
{
	size_t	i;

	i = 0;
	while (split_arr[i])
	{
		free(split_arr[i]);
		i++;
	}
}

static int	count_str(const char *s, char c, int hor_i)
{
	size_t	i;

	i = 0;
	while (s[hor_i] != c && s[hor_i] != '\0')
	{
		hor_i++;
		i++;
	}
	return (i);
}

static char	**fill_spl_arr(char **split_arr, int splits, const char *s, int c)
{
	int	hor_i;
	int	vert_i;
	int	length;

	hor_i = 0;
	vert_i = 0;
	length = 0;
	while (vert_i < splits - 1)
	{
		while (s[hor_i] == c)
			hor_i++;
		length = count_str(s, c, hor_i);
		split_arr[vert_i] = (char *)ft_calloc(length + 1, sizeof(char));
		if (!split_arr)
			return (NULL);
		ft_strlcpy(split_arr[vert_i], &s[hor_i], length + 1);
		hor_i += length;
		vert_i++;
	}
	split_arr[vert_i] = NULL;
	return (split_arr);
}

static int	ft_count_splits_pipex(char const *s, char c)
{
	int		counter;
	int		len;
	int		i;

	counter = 0;
	len = ft_strlen(s);
	i = 0;
	while (i < len)
	{
		if (s[i] != c)
			counter++;
		while (s[i] != c && i < len)
			i++;
		i++;
	}
	return (counter + 1);
}

char	**ft_split_pipex(char const *s, char c)
{
	char	**split_arr;
	int		splits;

	if (!s)
		return (NULL);
	splits = ft_count_splits_pipex(s, c);
	split_arr = (char **)ft_calloc(splits + 1, sizeof(char *));
	if (!split_arr)
		return (NULL);
	if (!*s || splits == 0)
		return (split_arr);
	if (!fill_spl_arr(split_arr, splits, s, c))
	{
		free_splitted_arr(split_arr);
		free(split_arr);
		return (NULL);
	}
	return (split_arr);
}
